...
class SaleTimesheetController(SaleTimesheetCustomerPortal):
    def _plan_prepare_values(self, projects):
        ...
        _where = (
            request.env["report.project.task.user"]
            .with_context(project_ids=projects.ids)
            ._where()
        )
        if projects:
            _where += """
                ...
                project_data[data.get("project_id")].update(
                    {
                        ...
                        + project_data[data.get("project_id")].get("schedule_variance"),
